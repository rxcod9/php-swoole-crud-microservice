...
use App\Core\Events\RequestErrorHandler;
...
final class HttpServerNew
{
    ...
    public function __construct(
        ...
        $requestDispatcher    = new RequestDispatcher($container);
        $requestMetricsLogger = new RequestMetricsLogger();
        $requestErrorHandler  = new RequestErrorHandler();

        // Now instantiate handler
        ...
        $this->server->on(
            'request',
            function ($request, $response) use ($router, $requestHandler): void {
                $requestContext = $requestHandler->createContext($request, $response);
                $requestHandler->handle($router, $requestContext, $this->server);
            }
        );
